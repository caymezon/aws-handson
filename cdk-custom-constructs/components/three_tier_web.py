...
class ThreeTierWebConstruct(Construct):
    ...
    --- カスタム Construct を作る理由 ---
    cdk-alb-ec2-rds では全リソースを1つのスタックファイルに書いた。
    本クラスでは「3層Webの定義」を Construct に切り出すことで:
      - 再利用性: ThreeTierStack や MultiEnvStack など複数スタックから呼び出せる
      - 責務の分離: インフラ定義とデプロイ単位（スタック）を分けて管理できる
      - テスト容易性: Construct 単体でユニットテストを書ける
    ...
    def __init__(self, scope: Construct, id: str, *, props: ThreeTierWebProps) -> None:
        ...
        prefix = f"n{props.employee_id}-cdk2"
        ...
        ssm.StringParameter(
            ...
            parameter_name=f"/n{props.employee_id}/cdk2/db-password",
        ...
        html_content = (
            ...
            "<p>Deployed via AWS CDK Custom Construct (ThreeTierWebConstruct).</p>\n"
            "</body></html>\n"
            "HTMLEOF"
        )
